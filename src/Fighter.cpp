...
void Fighter::notify_collision(GameObject & object) {
	  ...
	  if (object.is("floor") and speed.y >= 0 and abs(floor_y - (box.y + box.height * 0.5)) < 10){
	  ...
	  }else if (object.is("player") and !is("dying") and not(object.is("dying"))) {
...
void Fighter::change_state(FighterState cstate) {
	  ...
	  if (cstate == CROUCH or cstate == CROUCH_ATK or cstate == SPECIAL_1 or cstate == JUMP_ATK_DOWN_FALLLOOP or cstate == JUMP_ATK_DOWN_DMG) {
