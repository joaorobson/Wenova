...
Flesh::Flesh(string skin, float x, float y, int cid, Fighter *cpartner)
    ...
    sprite[JUMP_ATK_DOWN_FALLLOOP] =
        Sprite(path + "jump_atk_down_fallloop.png", 3, 10);
...
void Flesh::check_idle(bool change, bool condition) {
    if ((speed.x == 0 and on_floor and not is_holding[DOWN_BUTTON] and
         not is_holding[BLOCK_BUTTON]) or
        condition) {
        if (change)
...
void Flesh::check_idle_atk_front(bool change, bool condition) {
    if ((pressed[ATTACK_BUTTON] and
         (is_holding[LEFT_BUTTON] or is_holding[RIGHT_BUTTON])) or
        condition) {
        speed.y = 0;
        if (change)
...
void Flesh::check_jump_atk_neutral(bool change) {
    if (is_holding[ATTACK_BUTTON] and not is_holding[DOWN_BUTTON] and
        not is_holding[UP_BUTTON]) {
        if (change)
...
void Flesh::check_special_2(bool) {
    ...
    additional_speed =
        (1.0 - (1.0 * get_remaining_life()) / MAX_LIFE) * INITIAL_SPEED * 0.5;
}
...
void Flesh::check_ultimate(bool) {
    if (pressed[ULTIMATE_BUTTON] and special == MAX_SPECIAL) {
        Game::get_instance().get_current_state().add_object(
            new FleshUltimateEffect(this, path + "ult_effect.png", "has_sprite",
                                    1));
        ultimate_sound.play();
    }
}
