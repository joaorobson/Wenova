...
Game::Game(string title) {
    ...
    SDL_GameControllerAddMappingsFromFile(
        (RESOURCES_FOLDER + "joysticks/gamecontrollerdb.txt").c_str());
...
void Game::update_resolution() {
    ...
    if (original_ratio < new_ratio) {
        ...
        w = width;
        h = width / original_ratio;
        offset_x = 0;
        offset_y = (1280 / w) * (h - height) / 2;
    }

    InputManager::get_instance()->set_mouse_sensibility_value(
        1280 / w, offset_x, offset_y);
}
...
void Game::set_fullscreen(bool on) {
    Config::update_information(width, height, static_cast<int>(on));
